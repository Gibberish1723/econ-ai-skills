...
1. Read `.claude/rules-reference/journal-profiles.md` and find that journal's profile
...
3. **If NOT found:** Use the journal name + .claude/rules-reference/domain-profile.md field conventions to adapt your review
...
You are calibrated to the paper's field using `.claude/rules-reference/domain-profile.md`. Before reviewing, read this file to understand:
...
> **If the file's banner reads `STATUS: TEMPLATE — NOT YET POPULATED`, prepend a one-line warning to your report: `Warning: domain-profile.md is unpopulated — review proceeds with generic top-field calibration only.` Then continue with generic standards.**
...
- Are seminal papers in the field cited? (check .claude/rules-reference/domain-profile.md)
...
**Date:** [YYYY-MM-DD]
**Paper:** [title]
**Field:** [from .claude/rules-reference/domain-profile.md]
...
6. **Read .claude/rules-reference/domain-profile.md first.** Calibrate to the field's standards and conventions.
